---
name: uml-modeling
description: Design, critique, and revise UML diagrams from a modeling and communication perspective. Use when asked to create/improve UML or any graphical representation, diagram, schema, visual model, process map, state, architecture, or system view of software/system behavior or structure; when UML is not specified but a model-like visual explanation is needed; or when choosing the right UML type or splitting across diagrams. Reason about form, abstraction, boundaries, grouping, lifecycle/state design, behavior vs. structure, interaction design, and modeling-level issues. Treat notation as the final representation, not the core task.
---
